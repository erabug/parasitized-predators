...
points(.3, 0.1, cex = 1.5, pch = 19) # make a dot on the k+m plot (600x500 image size)
points(.03, 5, cex = 1.5, pch = 19) # make a dot on the m+b plot (600x500 image size)
points(.3, 5, cex = 1.5, pch = 19) # make a dot on the k+b plot (600x500 image size)
...
getGP <- function(state, times, parameters){
    last <- length(eval(times))   
    mRange <- seq(0, 1, 0.1)
    kRange <- bRange <- range <- seq(0, 10, 1)    
    Gvalues <- data.frame()
...
contourPlot <- function(Gvalues, xpar, ypar) {
    xlim <- ylim <- 0:10
    par <- c(xpar, ypar)
    ifelse(xpar == "m", {xlim <- unique(Gvalues$m); xlab <- "encounter rate (m)"}, 
           ifelse(xpar == "k", xlab <- "inverse predator lethality (k)", 
                  xlab <- "value of vigilance (b)"))
    ifelse(ypar == "m", {ylim <- unique(Gvalues$m); ylab <- "encounter rate (m)"}, 
           ifelse(ypar == "k", ylab <- "inverse predator lethality (k)", 
                  ylab <- "value of vigilance (b)"))
